...
class Source(models.Model):
    ...
    cpu_limit = models.IntegerField(
              ...
              help_text='Percentage  of  CPU allowed. Defaults to 100%')
...
class Configuration(models.Model):
    ...
    max_gaps_by_analysis = models.PositiveSmallIntegerField(
                         default=10,
                         validators=[MinValueValidator(1)],
                         help_text='If the number of gaps inside the window of '
                                   'analysis is over this threshold, they will '
                                   'be merged into 1 gap starting with the '
                                   'first gap starttime and ending with the '
                                   'last gap endtime')
    ...
    crontime = models.IntegerField(
             default=0,
             help_text="In case of a daily analysis, the software be executed "
                       "at that hour",
             validators=[MinValueValidator(0),MaxValueValidator(23)],
             verbose_name="Custom hour for crontab execution",
             )
...
class GapList(models.Model):
    ...
    force_source = models.BooleanField(
                 default=False,
                 help_text=('Check to force data from source(s). '
                            'This will erase and replace data in final archive.'
                            ' If unchecked, request will be created to fill '
                            'existing gaps in final archive.'
                            ))
...
class Request(models.Model):
    ...
    # forcing = models.BooleanField(default=False)
