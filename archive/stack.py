...
def update_availability(source, filename, dataformat):
    ...
    if source_avail.exists():
        ...
        # See archive.stats to see how this works
...
def create_requests(config, gap_list, source_list=None):
    ...
    Input :
        config : a archive.Configuration object
        gap_list : list of `archive.Gap` objects
    ...
    for gap in gaps:
        ...
        # get or create datafiles
        n,s,l,c = gap.nslc.code.split(".")
        nslc_qs = NSLC.objects.filter(pk=gap.nslc.pk)
        file_list = data_structure.get_filelist(
                  config.archive, nslc_qs,
                  gap_start,
                  gap_end)
        if datetime.date(gap_start) < datetime.date(gap_end):
            for day in toolbox.daterange(gap_start,gap_end):
                year = day.strftime("%Y")
                jday = day.strftime("%j")
                ...
                if key in file_list:
                    ...
                    if DataFile.objects.filter(
                        key=key, filename=filename).exists():
                        file = DataFile.objects.get(key=key, filename=filename)
                    else:
                        file = DataFile(key=key, filename=filename,
                        modif_time=datetime.timestamp(now),
                        stream_starttime = gap.starttime,
                        stream_endtime = gap.endtime)
                        file.save()
                    gap.files.add(file)
                    gap.save()
        else:
            year = gap_start.strftime("%Y")
            jday = gap_start.strftime("%j")
            key = "%s.%s.%s.%s.%s.%s" %(year,n,s,l,c,jday)
            if key in file_list:
                filename = file_list[key]
                if DataFile.objects.filter(
...
def execute_request(request, config):
    ...
    if request.gap.nslc.sta.multiplexing:
        ...
        pk_list = list()
        for gap in gap_queryset.all():
            for file in gap.files.all():
                pk_list.append(file.pk)
        gap_files = DataFile.objects.filter(pk__in=pk_list)
    else:
    ...
    try:
        for postfile in request.postfile.all():
            ...
            r = plugin.read(postfile.filename, tempdir_source, data_format,
                            blocksize, compression, connect_infos, cpu_limit,
                            temp_log, verbose)
            ...
            for path, subdirs, files in os.walk(tempdir_source):
                for name in files:
                    ...
                    patch_string = str(priority) + ";" + os.path.join(path, name)
                    patch_files.append(patch_string)
...
#
#
#     availability : `archive.models.SourceAvailability` objects
